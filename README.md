# Anand Shete

Backend developer focusing on scalable systems, distributed architecture, and high-performance APIs.

- Checkout my [Portfolio](https://www.anandshete.dev).
- Follow me on [LinkedIn](https://www.linkedin.com/in/anand-shete).

## Tech Stack

### Languages

[![My Skills](https://skillicons.dev/icons?i=cpp,python,ts)](https://skillicons.dev)

### Front-End

[![My Skills](https://skillicons.dev/icons?i=html,css,tailwind,js,react)](https://skillicons.dev)

### Back-End

[![My Skills](https://skillicons.dev/icons?i=nodejs,ts,express,fastapi)](https://skillicons.dev)

### Database and ORMs

[![My Skills](https://skillicons.dev/icons?i=mongodb,postgres,prisma)](https://skillicons.dev)

### Libraries and Tools

[![My Skills](https://skillicons.dev/icons?i=npm,git,postman,rabbitmq,github,docker)](https://skillicons.dev)

### Deployment

[![My Skills](https://skillicons.dev/icons?i=aws,gcp,vercel,nginx)](https://skillicons.dev)

### Operating System

[![My Skills](https://skillicons.dev/icons?i=windows,linux,ubuntu,bash,vim)](https://skillicons.dev)

---

## Notable Work

### [2D Metaverse](https://metaverse.anandshete.dev/)

- Built a real-time multiplayer environment using WebSockets (Socket.IO)
- Built an Agentic RAG pipeline to retrieve notes from S3 using AWS Lambda
- Integrated WebRTC for peer-to-peer media communication
- Designed event-driven architecture for synchronized player state

### [Blogify](https://blogify.anandshete.dev/)

- Developed a scalable blogging platform with a focus on backend performance and reliability
- Designed RESTful APIs for content creation, editing, and publishing workflows
- Built a Github Actions CD pipeline for frontend and backend
- Optimized database queries and caching strategies for faster content delivery

### [Markwise](https://markwise.anandshete.dev/)

- Built QR-based attendance system for 500+ users
- Implemented JWT authentication and secure session handling
- Optimized data flow between frontend and backend

---

Feel free to reach out.
